...
export default {
		...
		setData() {
			...
			if (this.$data.trend === "up") {
				...
				if (nextVal > 9) {
					...
					this.$data.arcGuideLowEnd[0] = Math.max(
						0,
						Math.min(3, nextVal)
					);
					this.$data.arcGuideMidEnd[0] = Math.max(
						3,
						Math.min(6, nextVal)
					);
					this.$data.arcGuideHighEnd[0] = Math.max(
						6,
						Math.min(9, nextVal)
					);
	...
	data() {
        ...
        <div style="width: 300px;text-align: center;">
          <p style="font-size: 20px;color: #545454;margin: 0;">合格率</p>
          <p style="font-size: 36px;color: #545454;margin: 0;">${data[0].value *
				10}%</p>
        </div>
      `
		};
